...
export const FinancePortal: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="p-7 rounded-[28px] bg-gradient-to-r from-purple-600 via-indigo-600 to-cyan-500 text-white shadow-xl shadow-purple-500/20 flex items-center justify-between">
        <div className="text-left">
          <span className="text-xs font-bold text-white/80 block uppercase">إجمالي المخاطر غير المودعة</span>
          <span className="text-2xl font-black text-amber-300">{totalOutstanding.toLocaleString('en-US')} ج.م</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-xs text-white/80 font-bold">مراجعة عهدة النقدية المعلقة لدى المناديب والتحقق من إيصالات البنك.</p>
          </div>
          ...
          <table className="w-full text-right text-xs">
            <thead>
              <tr className="border-b border-purple-100/60 text-slate-400 uppercase text-[11px] font-black">
                ...
                <th className="py-3.5 px-4">محصل اليوم</th>
                <th className="py-3.5 px-4">مودع في البنك</th>
                <th className="py-3.5 px-4">العهدة المعلقة</th>
                <th className="py-3.5 px-4">مستوى الخطورة</th>
                <th className="py-3.5 px-4 text-left">إجراء الخزينة</th>
              </tr>
            </thead>
              ...
              {MOCK_REPRESENTATIVES.map((rep) => {
                ...
                return (
                  <tr key={rep.id} className="hover:bg-purple-50/50">
                    ...
                    <td className="py-3.5 px-4 font-bold text-slate-600">{rep.territory}</td>
                    ...
                    <td className="py-3.5 px-4 text-left">
                      <button
                        ...
                        className={`px-4 py-2 rounded-full font-extrabold text-xs transition-all shadow-sm ${
                          isReconciled
