...
export const RepMyPortal: React.FC = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-300">
      {/* Rep Header Greeting */}
      <div className="p-5 sm:p-7 rounded-[24px] sm:rounded-[28px] bg-gradient-to-r from-purple-600 via-indigo-600 to-cyan-500 text-white shadow-xl shadow-purple-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 order-2 sm:order-1">
          ...
          <div className="text-right">
            <div className="flex items-center gap-2 justify-start sm:justify-end">
              <span className="text-[10px] font-mono font-bold bg-white/20 text-white px-2.5 py-0.5 rounded-full">{rep.employeeCode}</span>
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-white/20 text-white">
                بوابة المندوب الطبي
              </span>
            </div>
            <h1 className="text-lg sm:text-xl font-black text-white tracking-tight mt-1 text-right">أهلاً، {rep.name}</h1>
            <p className="text-[11px] sm:text-xs text-white/80 font-bold text-right">المنطقة: {rep.territory} | السيارة: {rep.vehiclePlate}</p>
          </div>
        </div>

        <button
          onClick={() => setCheckedIn(!checkedIn)}
          className={`w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 rounded-full font-extrabold text-xs shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer order-1 sm:order-2 ${
            checkedIn
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-2 p-4 sm:p-6 rounded-[24px] sm:rounded-[28px] bg-white/90 backdrop-blur-md border border-purple-100/60 shadow-sm shadow-purple-900/5 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-purple-100/60 pb-3 gap-2">
            ...
            <span className="text-xs font-extrabold text-purple-700 bg-purple-50 px-3 py-1 rounded-full border border-purple-100 self-start sm:self-auto">
          ...
          <div className="space-y-3">
            {visitsList.map((visit) => (
              ...
              >
                ...
                <div>
                  {visit.status === 'COMPLETED' && (
                    <span className="text-[11px] font-extrabold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100 inline-block">
                      زيارة مؤكدة
                    </span>
                  )}
                  {visit.status === 'IN_PROGRESS' && (
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2 rounded-full bg-purple-600 text-white text-xs font-extrabold shadow-md shadow-purple-500/20 hover:bg-purple-700 transition-all flex items-center justify-center gap-1 cursor-pointer"
                    >
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="p-4 sm:p-6 rounded-[24px] sm:rounded-[28px] bg-white/90 backdrop-blur-md border border-purple-100/60 shadow-sm shadow-purple-900/5 space-y-3">
          ...
          <div className="p-4 sm:p-6 rounded-[24px] sm:rounded-[28px] bg-white/90 backdrop-blur-md border border-purple-100/60 shadow-sm shadow-purple-900/5 space-y-3">
            ...
            <form onSubmit={handleCashCollection} className="space-y-3">
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow-md shadow-emerald-500/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
              >
      ...
      {orderModalOpen && typeof window !== 'undefined' && require('react-dom').createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
          <div className="w-full max-w-md bg-white border border-purple-100 rounded-[28px] p-4 sm:p-6 shadow-2xl space-y-4 my-auto">
            <div className="flex items-center justify-between border-b border-purple-100 pb-3">
              <button onClick={() => setOrderModalOpen(false)} className="text-slate-400 hover:text-purple-600 font-bold p-1 cursor-pointer">
                ✕
              </button>
            ...
            <form onSubmit={handleOrderSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3.5 rounded-full bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-xs shadow-lg shadow-purple-500/20 transition-all cursor-pointer"
              >
                إرسال الطلب للنظام
              </button>
              {orderSuccess && (
